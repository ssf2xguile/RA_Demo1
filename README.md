...
※この手順では、最大で4つのターミナルを使用します。
- ターミナル1：コネクテッドカーシステムの起動・停止用
- ターミナル2：コネクテッドカーシステムのログ収集用
- ターミナル3：コネクテッドカーシステムのリソース使用状況の取得用
- ターミナル4：メトリクス取得(curl)またはタイムアウト数モニターアプリケーションの実行用
...
ここでは新しくターミナル3を開き、環境に応じた方法で monitor.log を作成します。
...
`app_server/main.py`での処理状況を確認したい場合は、ターミナル1でコンテナが動作している状態で、ターミナル4を開き次のAPIを叩きます。
...
```
補足ですが、ターミナル4でコマンド実行する際、ディレクトリはどこでも構いません。
...
ターミナル4を起動しディレクトリを移動します。そして仮想環境を有効にします。
1. ターミナル1でコネクテッドカーシステムを起動した状態にします。
2. ターミナル4で次のコマンドを実行します。
```cmd
