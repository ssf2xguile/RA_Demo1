...
def heavy_cpu_task():
    """CPUに高い負荷をかける同期関数"""
    print(f"Starting heavy CPU task...")
    result = 0
    # 計算量を調整し、10件の同時リクエストでは重すぎないようにする
...
async def start_climate(data: dict, request: Request):
    ...
    print(f"AppServer: [Req ID: {request_id}] Received.")

    try:
        # CPU負荷の高い同期処理をワーカースレッドで実行
    ...
    # タイムアウト後も処理は続行され、車両シミュレータにコマンドを送信
    try:
