...
def _rec_timeout():
    now = time.time()
    _timeouts.append((now, 1))
    while _timeouts and now - _timeouts[0][0] > METRICS_WINDOW_S:
        _timeouts.popleft()
...
def _recent():
    ...
    while _timeouts and now - _timeouts[0][0] > METRICS_WINDOW_S:
        _timeouts.popleft()
    a = sum(v for _, v in _arrivals)
    t = sum(v for _, v in _timeouts)
